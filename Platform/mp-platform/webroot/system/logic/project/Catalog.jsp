...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/logic/project/Project.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='业务-项目' icon='sys.pst.mgr' target='frmMain' page='list@#/logic/project/Project.wa?form_name=system.project.ProjectList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.logic.project'/>
<!-- Body begin ---------------------------------------------->
