...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/logic/event/Event.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='业务-事件分组' icon='sys.pst.mgr' target='frmMain' page='list@#/logic/event/Event.wa?form_name=system.logic.event.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.logic.event'/>
<!-- Body begin ---------------------------------------------->
