...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/database/logicunit/LogicUnit.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='逻辑单元列表' icon='sys.pst.mgr' target='frmMain' page='list@#/database/logicunit/LogicUnit.wa?form_name=database.logicunit.LogicUnitList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.database.logicunit'/>
<!-- Body begin ---------------------------------------------->
