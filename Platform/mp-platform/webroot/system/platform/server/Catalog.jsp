...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/platform/server/Server.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='服务器列表' icon='sys.pst.mgr' target='frmMain' page='list@#/platform/Server.wa?form_name=system.platform.server.ServerList'/>
