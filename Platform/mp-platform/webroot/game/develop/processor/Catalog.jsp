...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/editor/processor/Processor.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='处理器列表' icon='sys.pst.mgr' target='frmMain' page='#/editor/processor/Processor.wa?do=list&amp;form_name=game.editor.processor.ProjectList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='game.editor.processor'/>
<!-- Body begin ---------------------------------------------->
