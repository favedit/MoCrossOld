...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/translate/Translate.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='计划-翻译' icon='sys.pst.mgr' target='frmMain' page='list@#/design/translate/Translate.wa?form_name=design.translate.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.design.translate'/>
<!-- Body begin ---------------------------------------------->
