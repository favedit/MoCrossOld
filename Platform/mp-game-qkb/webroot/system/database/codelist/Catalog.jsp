...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/database/codelist/CodeList.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='定义列表' icon='sys.pst.mgr' target='frmMain' page='list@#/database/codelist/CodeList.wa?form_name=database.codelist.CodeListList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.database.codelist'/>
<!-- Body begin ---------------------------------------------->
