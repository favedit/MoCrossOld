...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/editor/entity/Entity.wa?do=update';
	fmMain.submit();
}
//----------------------------------------------------------
...
function onSearch(){
   ...
   if(s){
      ...
      tree.service = 'search@game.editor.entity';
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='实体列表' icon='sys.pst.mgr' target='frmMain' page='#/editor/entity/Entity.wa?do=list&amp;form_name=game.editor.entity.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='game.editor.entity'/>
<!-- Body begin ---------------------------------------------->
