...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/editor/storage/Storage.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='存储器列表' icon='sys.pst.mgr' target='frmMain' page='#/editor/storage/Storage.wa?do=list&amp;form_name=game.editor.storage.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='game.editor.storage'/>
<!-- Body begin ---------------------------------------------->
