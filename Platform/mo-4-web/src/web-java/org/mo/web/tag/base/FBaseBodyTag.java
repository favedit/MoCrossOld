 ...
 */
public abstract class FBaseBodyTag
      extends FAbstractHtmlTag
{
   ...
   public void appendHtml(){
      ...
      /// 如果背景颜色不空，则添加该属性
      if(null != _bgcolor){
         _writer.appendAttribute("bgcolor", _bgcolor, true);
      }
      /// 如果背景不空，则添加该属性
      if(null != _background){
         _writer.appendAttribute("background", _background, true);
      }
      /// 如果滚动条不空，则添加该属性
      if(null != _scroll){
         _writer.appendAttribute("scroll", _scroll, true);
      }
      /// 如果样式不空，则添加该属性
      if(null != _style){
