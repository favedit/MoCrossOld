...
// <T>分割条对象的XML节点基类。</T>
//============================================================
public abstract class XBaseSplit
      extends FXmlObject
      implements
         XControlFace
{
   ...
   // 显示方式的名称定义
   public static final String PTY_DISP_TYPE = "disp_type";

   // 显示风格的名称定义
   public static final String PTY_DISP_STYLE = "disp_style";

   // 默认展开的名称定义
   public static final String PTY_EDIT_EXTEND = "edit_extend";
   ...
   // 显示方式的定义
   @AName("disp_type")
   protected String _dispType;

   // 显示风格的定义
   @AName("disp_style")
   protected String _dispStyle;

   // 默认展开的定义
   @AName("edit_extend")
   protected String _editExtend;
   ...
   // @param value 显示方式
   //============================================================
   public void setDispType(String value){
      _dispType = value;
   }
   ...
   // @param value 显示风格
   //============================================================
   public void setDispStyle(String value){
      _dispStyle = value;
   }
   ...
   // @param value 默认展开
   //============================================================
   public void setEditExtend(String value){
      _editExtend = value;
   }
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getIcon();
      }else if(PTY_DISP_TYPE.equalsIgnoreCase(name)){
         return getDispType();
      }else if(PTY_DISP_STYLE.equalsIgnoreCase(name)){
         return getDispStyle();
      }else if(PTY_EDIT_EXTEND.equalsIgnoreCase(name)){
         return getEditExtend();
      }
      return null;
   }
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("disp_type")){
            setDispType(config.get(PTY_DISP_TYPE));
         }
         if(config.contains("disp_style")){
            setDispStyle(config.get(PTY_DISP_STYLE));
         }
         if(config.contains("edit_extend")){
            setEditExtend(config.get(PTY_EDIT_EXTEND));
         }
         ...
         if(RString.isEmpty(getDispStyle())){
            if(config.contains("disp_style")){
               setDispStyle(config.get(PTY_DISP_STYLE));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getDispType())){
            config.set(PTY_DISP_TYPE, getDispType());
         }
         if(RString.isNotEmpty(getDispStyle())){
            config.set(PTY_DISP_STYLE, getDispStyle());
         }
         if(RString.isNotEmpty(getEditExtend())){
            config.set(PTY_EDIT_EXTEND, getEditExtend());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sDispType = getDispType();
         if(RString.isNotEmpty(sDispType)){
            config.set(PTY_DISP_TYPE, sDispType);
         }
         String sDispStyle = getDispStyle();
         if(RString.isNotEmpty(sDispStyle)){
            config.set(PTY_DISP_STYLE, sDispStyle);
         }
         String sEditExtend = getEditExtend();
         if(RString.isNotEmpty(sEditExtend)){
            config.set(PTY_EDIT_EXTEND, sEditExtend);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
