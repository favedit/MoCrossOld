         ...
         XEditControlFace
{
   ...
   // 数据服务的名称定义
   public static final String PTY_DATA_SERVICE = "data_service";

   // flash类型的名称定义
   public static final String PTY_FLASH_TYPE = "flash_type";
   ...
   // 数据服务的定义
   @AName("data_service")
   protected String _dataService;

   // flash类型的定义
   @AName("flash_type")
   protected String _flashType;
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getTypeAble();
      }else if(PTY_DATA_SERVICE.equalsIgnoreCase(name)){
         return getDataService();
      }else if(PTY_FLASH_TYPE.equalsIgnoreCase(name)){
         return getFlashType();
      }
      return null;
   }
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("data_service")){
            setDataService(config.get(PTY_DATA_SERVICE));
         }
         if(config.contains("flash_type")){
            setFlashType(config.get(PTY_FLASH_TYPE));
         }
         ...
         if(RString.isEmpty(getDataService())){
            if(config.contains("data_service")){
               setDataService(config.get(PTY_DATA_SERVICE));
            }
         }
         if(RString.isEmpty(getFlashType())){
            if(config.contains("flash_type")){
               setFlashType(config.get(PTY_FLASH_TYPE));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getDataService())){
            config.set(PTY_DATA_SERVICE, getDataService());
         }
         if(RString.isNotEmpty(getFlashType())){
            config.set(PTY_FLASH_TYPE, getFlashType());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sDataService = getDataService();
         if(RString.isNotEmpty(sDataService)){
            config.set(PTY_DATA_SERVICE, sDataService);
         }
         String sFlashType = getFlashType();
         if(RString.isNotEmpty(sFlashType)){
            config.set(PTY_FLASH_TYPE, sFlashType);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
