         ...
         XDescSelectFace
{
   ...
   // 允许空值的名称定义
   public static final String PTY_EDIT_EMPTY = "edit_empty";

   // 编辑引用的名称定义
   public static final String PTY_EDIT_REFER = "edit_refer";

   // 可输入的名称定义
   public static final String PTY_EDIT_CHECK = "edit_check";

   // 对齐方式的名称定义
   public static final String PTY_EDIT_ALIGN = "edit_align";

   // 动态编辑的名称定义
   public static final String PTY_EDIT_DYNAMIC = "edit_dynamic";
   ...
   // 允许空值的定义
   @AName("edit_empty")
   protected Boolean _editEmpty = Boolean.FALSE;
   ...
   // 可输入的定义
   @AName("edit_check")
   protected Boolean _editCheck = Boolean.FALSE;
   ...
   // @param value 允许空值
   //============================================================
   public void setEditEmpty(Boolean value){
      _editEmpty = value;
   }
   ...
   // @param value 可输入
   //============================================================
   public void setEditCheck(Boolean value){
      _editCheck = value;
   }
   ...
   // @param value 动态编辑
   //============================================================
   public void setEditDynamic(String value){
      _editDynamic = value;
   }
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getTypeAble();
      }else if(PTY_EDIT_EMPTY.equalsIgnoreCase(name)){
         return RBoolean.toString(getEditEmpty());
      }else if(PTY_EDIT_REFER.equalsIgnoreCase(name)){
         return getEditRefer();
      }else if(PTY_EDIT_CHECK.equalsIgnoreCase(name)){
         return RBoolean.toString(getEditCheck());
      }else if(PTY_EDIT_ALIGN.equalsIgnoreCase(name)){
         return getEditAlign();
      }else if(PTY_EDIT_DYNAMIC.equalsIgnoreCase(name)){
         return getEditDynamic();
      }
      return null;
   }
   ...
   public void innerSet(String name, String value){
      if(RString.isEmpty(name)){
         ...
         setEditEmpty(RBoolean.parse(value));
      }else if(PTY_EDIT_REFER.equalsIgnoreCase(name)){
         setEditRefer(value);
      }else if(PTY_EDIT_CHECK.equalsIgnoreCase(name)){
         setEditCheck(RBoolean.parse(value));
      }else if(PTY_EDIT_ALIGN.equalsIgnoreCase(name)){
         setEditAlign(value);
      }else if(PTY_EDIT_DYNAMIC.equalsIgnoreCase(name)){
         setEditDynamic(value);
      }
   }
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("edit_empty")){
            setEditEmpty(RBoolean.parse(config.get(PTY_EDIT_EMPTY)));
         }
         if(config.contains("edit_refer")){
            setEditRefer(config.get(PTY_EDIT_REFER));
         }
         if(config.contains("edit_check")){
            setEditCheck(RBoolean.parse(config.get(PTY_EDIT_CHECK)));
         }
         if(config.contains("edit_align")){
            setEditAlign(config.get(PTY_EDIT_ALIGN));
         }
         if(config.contains("edit_dynamic")){
            setEditDynamic(config.get(PTY_EDIT_DYNAMIC));
         }
         ...
         if(RString.isEmpty(getEditDynamic())){
            if(config.contains("edit_dynamic")){
               setEditDynamic(config.get(PTY_EDIT_DYNAMIC));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RBoolean.parse(getEditEmpty())){
            config.set(PTY_EDIT_EMPTY, RBoolean.toString(getEditEmpty()));
         }
         if(RString.isNotEmpty(getEditRefer())){
            config.set(PTY_EDIT_REFER, getEditRefer());
         }
         if(RBoolean.parse(getEditCheck())){
            config.set(PTY_EDIT_CHECK, RBoolean.toString(getEditCheck()));
         }
         if(RString.isNotEmpty(getEditAlign())){
            config.set(PTY_EDIT_ALIGN, getEditAlign());
         }
         if(RString.isNotEmpty(getEditDynamic())){
            config.set(PTY_EDIT_DYNAMIC, getEditDynamic());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         Boolean bEditEmpty = getEditEmpty();
         if(RBoolean.parse(bEditEmpty)){
            config.set(PTY_EDIT_EMPTY, RBoolean.toString(bEditEmpty));
         }
         String sEditRefer = getEditRefer();
         if(RString.isNotEmpty(sEditRefer)){
            config.set(PTY_EDIT_REFER, sEditRefer);
         }
         Boolean bEditCheck = getEditCheck();
         if(RBoolean.parse(bEditCheck)){
            config.set(PTY_EDIT_CHECK, RBoolean.toString(bEditCheck));
         }
         String sEditAlign = getEditAlign();
         if(RString.isNotEmpty(sEditAlign)){
            config.set(PTY_EDIT_ALIGN, sEditAlign);
         }
         String sEditDynamic = getEditDynamic();
         if(RString.isNotEmpty(sEditDynamic)){
            config.set(PTY_EDIT_DYNAMIC, sEditDynamic);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
