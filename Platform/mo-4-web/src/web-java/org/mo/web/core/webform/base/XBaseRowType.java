...
// <T>行类型对象的XML节点基类。</T>
//============================================================
public abstract class XBaseRowType
      extends FXmlObject
      implements
         XControlFace
{
   // 名称定义
   public static final String NAME = "RowType";
   ...
   // 表单条件的名称定义
   public static final String PTY_FORM_WHERE = "form_where";

   // 表单排序的名称定义
   public static final String PTY_FORM_ORDER = "form_order";

   // 查询有效的名称定义
   public static final String PTY_FORM_RESEARCH = "form_research";
   ...
   // 查询有效的定义
   @AName("form_research")
   protected String _formResearch;
   ...
   // @param value 表单条件
   //============================================================
   public void setFormWhere(String value){
      _formWhere = value;
   }
   ...
   // @return 查询有效
   //============================================================
   public String getFormResearch(){
      return _formResearch;
   }
   ...
   // @param value 查询有效
   //============================================================
   public void setFormResearch(String value){
      _formResearch = value;
   }
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getFormName();
      }else if(PTY_FORM_WHERE.equalsIgnoreCase(name)){
         return getFormWhere();
      }else if(PTY_FORM_ORDER.equalsIgnoreCase(name)){
         return getFormOrder();
      }else if(PTY_FORM_RESEARCH.equalsIgnoreCase(name)){
         return getFormResearch();
      }
      return null;
   }
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("form_where")){
            setFormWhere(config.get(PTY_FORM_WHERE));
         }
         if(config.contains("form_order")){
            setFormOrder(config.get(PTY_FORM_ORDER));
         }
         if(config.contains("form_research")){
            setFormResearch(config.get(PTY_FORM_RESEARCH));
         }
         ...
         if(RString.isEmpty(getFormName())){
            if(config.contains("form_name")){
               setFormName(config.get(PTY_FORM_NAME));
            }
         }
         if(RString.isEmpty(getFormWhere())){
            if(config.contains("form_where")){
               setFormWhere(config.get(PTY_FORM_WHERE));
            }
         }
         if(RString.isEmpty(getFormOrder())){
            if(config.contains("form_order")){
               setFormOrder(config.get(PTY_FORM_ORDER));
            }
         }
         if(RString.isEmpty(getFormResearch())){
            if(config.contains("form_research")){
               setFormResearch(config.get(PTY_FORM_RESEARCH));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getFormWhere())){
            config.set(PTY_FORM_WHERE, getFormWhere());
         }
         if(RString.isNotEmpty(getFormOrder())){
            config.set(PTY_FORM_ORDER, getFormOrder());
         }
         if(RString.isNotEmpty(getFormResearch())){
            config.set(PTY_FORM_RESEARCH, getFormResearch());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sFormWhere = getFormWhere();
         if(RString.isNotEmpty(sFormWhere)){
            config.set(PTY_FORM_WHERE, sFormWhere);
         }
         String sFormOrder = getFormOrder();
         if(RString.isNotEmpty(sFormOrder)){
            config.set(PTY_FORM_ORDER, sFormOrder);
         }
         String sFormResearch = getFormResearch();
         if(RString.isNotEmpty(sFormResearch)){
            config.set(PTY_FORM_RESEARCH, sFormResearch);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
