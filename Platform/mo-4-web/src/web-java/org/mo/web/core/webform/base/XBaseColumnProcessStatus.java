...
import org.mo.web.core.webform.control.XDescProcessStatusFace;

//============================================================
// <T>处理状态列对象的XML节点基类。</T>
//============================================================
public abstract class XBaseColumnProcessStatus
      extends FXmlObject
      implements
         XColumnEditControlFace,
         XDescProcessStatusFace
{
   // 名称定义
   public static final String NAME = "ColumnProcessStatus";
   ...
   // 处理级别颜色的名称定义
   public static final String PTY_LEVEL_COLORS = "level_colors";

   // 处理级别图片的名称定义
   public static final String PTY_LEVEL_IMAGES = "level_images";

   // 级别显示起始位置的名称定义
   public static final String PTY_STEP_START = "step_start";

   // 单个级别显示宽度的名称定义
   public static final String PTY_STEP_WIDTH = "step_width";

   // 显示级别的个数的名称定义
   public static final String PTY_STEP_COUNT = "step_count";
   ...
   // 处理级别颜色的定义
   @AName("level_colors")
   protected String _levelColors;

   // 处理级别图片的定义
   @AName("level_images")
   protected String _levelImages;

   // 级别显示起始位置的定义
   @AName("step_start")
   protected String _stepStart;
   ...
   // @param value 处理级别颜色
   //============================================================
   public void setLevelColors(String value){
      _levelColors = value;
   }
   ...
   // @param value 处理级别图片
   //============================================================
   public void setLevelImages(String value){
      _levelImages = value;
   }
   ...
   //============================================================
   public void setStepStart(String value){
      _stepStart = value;
   }
   ...
   // @param value 显示级别的个数
   //============================================================
   public void setStepCount(String value){
      _stepCount = value;
   }
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getOrderAble();
      }else if(PTY_LEVEL_COLORS.equalsIgnoreCase(name)){
         return getLevelColors();
      }else if(PTY_LEVEL_IMAGES.equalsIgnoreCase(name)){
         return getLevelImages();
      }else if(PTY_STEP_START.equalsIgnoreCase(name)){
         return getStepStart();
      }else if(PTY_STEP_WIDTH.equalsIgnoreCase(name)){
         return getStepWidth();
      }else if(PTY_STEP_COUNT.equalsIgnoreCase(name)){
         return getStepCount();
      }
      return null;
   }
   ...
   public void innerSet(String name, String value){
      if(RString.isEmpty(name)){
         ...
         setLevelColors(value);
      }else if(PTY_LEVEL_IMAGES.equalsIgnoreCase(name)){
         setLevelImages(value);
      }else if(PTY_STEP_START.equalsIgnoreCase(name)){
         setStepStart(value);
      }else if(PTY_STEP_WIDTH.equalsIgnoreCase(name)){
         setStepWidth(value);
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("level_colors")){
            setLevelColors(config.get(PTY_LEVEL_COLORS));
         }
         if(config.contains("level_images")){
            setLevelImages(config.get(PTY_LEVEL_IMAGES));
         }
         if(config.contains("step_start")){
            setStepStart(config.get(PTY_STEP_START));
         }
         if(config.contains("step_width")){
            setStepWidth(config.get(PTY_STEP_WIDTH));
         }
         if(config.contains("step_count")){
            setStepCount(config.get(PTY_STEP_COUNT));
         }
         ...
         if(RString.isEmpty(getLevelColors())){
            if(config.contains("level_colors")){
               setLevelColors(config.get(PTY_LEVEL_COLORS));
            }
         }
         if(RString.isEmpty(getLevelImages())){
            if(config.contains("level_images")){
               setLevelImages(config.get(PTY_LEVEL_IMAGES));
            }
         }
         if(RString.isEmpty(getStepStart())){
            if(config.contains("step_start")){
               setStepStart(config.get(PTY_STEP_START));
            }
         }
         ...
         if(RString.isEmpty(getStepCount())){
            if(config.contains("step_count")){
               setStepCount(config.get(PTY_STEP_COUNT));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getLevelColors())){
            config.set(PTY_LEVEL_COLORS, getLevelColors());
         }
         if(RString.isNotEmpty(getLevelImages())){
            config.set(PTY_LEVEL_IMAGES, getLevelImages());
         }
         if(RString.isNotEmpty(getStepStart())){
            config.set(PTY_STEP_START, getStepStart());
         }
         if(RString.isNotEmpty(getStepWidth())){
            config.set(PTY_STEP_WIDTH, getStepWidth());
         }
         if(RString.isNotEmpty(getStepCount())){
            config.set(PTY_STEP_COUNT, getStepCount());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sLevelColors = getLevelColors();
         if(RString.isNotEmpty(sLevelColors)){
            config.set(PTY_LEVEL_COLORS, sLevelColors);
         }
         String sLevelImages = getLevelImages();
         if(RString.isNotEmpty(sLevelImages)){
            config.set(PTY_LEVEL_IMAGES, sLevelImages);
         }
         String sStepStart = getStepStart();
         if(RString.isNotEmpty(sStepStart)){
            config.set(PTY_STEP_START, sStepStart);
         }
         String sStepWidth = getStepWidth();
         if(RString.isNotEmpty(sStepWidth)){
            config.set(PTY_STEP_WIDTH, sStepWidth);
         }
         String sStepCount = getStepCount();
         if(RString.isNotEmpty(sStepCount)){
            config.set(PTY_STEP_COUNT, sStepCount);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
