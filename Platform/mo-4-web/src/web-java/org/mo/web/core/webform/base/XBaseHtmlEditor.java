...
//============================================================
// <T>html编辑框对象的XML节点基类。</T>
//============================================================
public abstract class XBaseHtmlEditor
      extends FXmlObject
      implements
         XEditControlFace
{
   ...
   // 自动换行的名称定义
   public static final String PTY_EDIT_OVERFLOW = "edit_overflow";

   // 最大长度的名称定义
   public static final String PTY_VALID_LENMAX = "valid_lenmax";

   // 最小长度的名称定义
   public static final String PTY_VALID_LENMIN = "valid_lenmin";
   ...
   // 自动换行的定义
   @AName("edit_overflow")
   protected String _editOverflow;
   ...
   // @return 自动换行
   //============================================================
   public String getEditOverflow(){
      return _editOverflow;
   }
   ...
   // @param value 自动换行
   //============================================================
   public void setEditOverflow(String value){
      _editOverflow = value;
   }
   ...
   // @param value 最小长度
   //============================================================
   public void setValidLenmin(String value){
      _validLenmin = value;
   }
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getTypeAble();
      }else if(PTY_EDIT_OVERFLOW.equalsIgnoreCase(name)){
         return getEditOverflow();
      }else if(PTY_VALID_LENMAX.equalsIgnoreCase(name)){
         return getValidLenmax();
      }else if(PTY_VALID_LENMIN.equalsIgnoreCase(name)){
         return getValidLenmin();
      }
      return null;
   }
   ...
   public void innerSet(String name, String value){
      if(RString.isEmpty(name)){
         ...
         setEditOverflow(value);
      }else if(PTY_VALID_LENMAX.equalsIgnoreCase(name)){
         setValidLenmax(value);
      }else if(PTY_VALID_LENMIN.equalsIgnoreCase(name)){
         setValidLenmin(value);
      }
   }
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("edit_overflow")){
            setEditOverflow(config.get(PTY_EDIT_OVERFLOW));
         }
         if(config.contains("valid_lenmax")){
            setValidLenmax(config.get(PTY_VALID_LENMAX));
         }
         if(config.contains("valid_lenmin")){
            setValidLenmin(config.get(PTY_VALID_LENMIN));
         }
         ...
         if(RString.isEmpty(getEditOverflow())){
            if(config.contains("edit_overflow")){
               setEditOverflow(config.get(PTY_EDIT_OVERFLOW));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getEditOverflow())){
            config.set(PTY_EDIT_OVERFLOW, getEditOverflow());
         }
         if(RString.isNotEmpty(getValidLenmax())){
            config.set(PTY_VALID_LENMAX, getValidLenmax());
         }
         if(RString.isNotEmpty(getValidLenmin())){
            config.set(PTY_VALID_LENMIN, getValidLenmin());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sEditOverflow = getEditOverflow();
         if(RString.isNotEmpty(sEditOverflow)){
            config.set(PTY_EDIT_OVERFLOW, sEditOverflow);
         }
         String sValidLenmax = getValidLenmax();
         if(RString.isNotEmpty(sValidLenmax)){
            config.set(PTY_VALID_LENMAX, sValidLenmax);
         }
         String sValidLenmin = getValidLenmin();
         if(RString.isNotEmpty(sValidLenmin)){
            config.set(PTY_VALID_LENMIN, sValidLenmin);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
