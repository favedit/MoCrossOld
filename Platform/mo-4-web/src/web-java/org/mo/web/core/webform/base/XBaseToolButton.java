         ...
         XToolButtonFace
{
   ...
   // 执行内容的名称定义
   public static final String PTY_EXECUTE = "execute";
   ...
   public String innerGet(String name){
      ...
      }else if(PTY_EXECUTE.equalsIgnoreCase(name)){
         return getExecute();
      }
      return null;
   }
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("execute")){
            setExecute(config.get(PTY_EXECUTE));
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getExecute())){
            config.set(PTY_EXECUTE, getExecute());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sExecute = getExecute();
         if(RString.isNotEmpty(sExecute)){
            config.set(PTY_EXECUTE, sExecute);
         }
      }else if(EXmlConfig.Default == type){
         String sPadLeft = getPadLeft();
         if(RString.isNotEmpty(sPadLeft)){
