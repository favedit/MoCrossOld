...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/editor/instance/Instance.wa?do=update';
	fmMain.submit();
}
//----------------------------------------------------------
...
function onSearch(){
   ...
   if(s){
      ...
      tree.service = 'search@game.editor.instance';
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='实体列表' icon='sys.pst.mgr' target='frmMain' page='#/editor/instance/Instance.wa?do=list&amp;form_name=game.editor.instance.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='game.editor.instance'/>
<!-- Body begin ---------------------------------------------->
