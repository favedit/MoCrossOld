...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/editor/message/Message.wa?do=update';
	fmMain.submit();
}
//----------------------------------------------------------
...
function onSearch(){
   ...
   if(s){
      ...
      tree.service = 'search@game.editor.message';
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='消息列表' icon='sys.pst.mgr' target='frmMain' page='#/editor/message/Message.wa?do=list&amp;form_name=game.editor.message.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<jc:tree name='xTree' source='game.editor.message'/>
<!-- Body begin ---------------------------------------------->
