...
function onNodeClick(sender, node){
	...
	if('collection' == type){
      ...
      var colNode = node.topNodeByType('collection');
		fmMain.sel_collection.value = colNode.get('dataset_name');
		fmMain.sel_component.value = node.uuid;
	}else{
		return;
	}
	fmMain.action = '<jh:context/>/database/dataset/Dataset.wa?do=update';
	fmMain.submit();
}
//----------------------------------------------------------
...
function onSearch(){
   ...
   if(s){
      ...
      tree.service = 'search@database.dataset';
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='数据集列表' icon='sys.pst.mgr' target='frmMain' page='list@#/database/dataset/Dataset.wa?form_name=database.dataset.DatasetList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.database.dataset'/>
<!-- Body begin ---------------------------------------------->
