...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/system/transfer/Transfer.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='传输设置' icon='sys.pst.mgr' target='frmMain' page='list@#/system/transfer/Transfer.wa?form_name=system.transfer.CsvImportList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.system.transfer'/>
<!-- Body begin ---------------------------------------------->
