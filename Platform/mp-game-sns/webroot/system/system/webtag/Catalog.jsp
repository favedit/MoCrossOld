...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/system/webtag/WebTag.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='系统-tag' icon='sys.pst.mgr' target='frmMain' page='list@#/system/webtag/WebTag.wa?form_name=system.system.webtag.TagLibList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.system.webtag'/>
<!-- Body begin ---------------------------------------------->
