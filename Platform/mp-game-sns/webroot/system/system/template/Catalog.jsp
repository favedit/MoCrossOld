...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/system/template/Template.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
    <ToolButtonText name='btnForm' label='模板列表' icon='sys.pst.mgr' target='frmMain' page='#/system/template/Template.wa?do=list&amp;form_name=system.template.TemplateList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.system.template'/>
<!-- Body begin ---------------------------------------------->
