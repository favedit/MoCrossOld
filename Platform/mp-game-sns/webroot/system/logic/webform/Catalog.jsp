...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/webtools/WebTools.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
	top.fstBody.cols = '400,*';
	// Toolbar
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='工具栏列表' icon='sys.pst.mgr' target='frmMain' page='#/design/webtools/WebTools.wa?do=list&amp;form_name=design.webtools.ToolBarList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.design.webtools'/>
<!-- Body begin ---------------------------------------------->
