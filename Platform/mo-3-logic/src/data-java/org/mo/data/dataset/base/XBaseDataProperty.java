...
// <T>属性视图对象的XML节点基类。</T>
//============================================================
public abstract class XBaseDataProperty
      extends FXmlObject
      implements
         XDatasetFace
{
   // 名称定义
   public static final String NAME = "DataProperty";
   ...
   // 设置表名称的名称定义
   public static final String PTY_PARENT_VALUE = "parent_value";

   // 记录编号的名称定义
   public static final String PTY_RECORD_CODE = "record_code";
   ...
   public String innerGet(String name){
      ...
      }else if(PTY_PARENT_VALUE.equalsIgnoreCase(name)){
         return getParentValue();
      }else if(PTY_RECORD_CODE.equalsIgnoreCase(name)){
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("parent_value")){
            setParentValue(config.get(PTY_PARENT_VALUE));
         }
         if(config.contains("record_code")){
            setRecordCode(config.get(PTY_RECORD_CODE));
         }
         ...
         if(RString.isEmpty(getParentValue())){
            if(config.contains("parent_value")){
               setParentValue(config.get(PTY_PARENT_VALUE));
            }
         }
         if(RString.isEmpty(getRecordCode())){
            if(config.contains("record_code")){
               setRecordCode(config.get(PTY_RECORD_CODE));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getParentValue())){
            config.set(PTY_PARENT_VALUE, getParentValue());
         }
         if(RString.isNotEmpty(getRecordCode())){
            config.set(PTY_RECORD_CODE, getRecordCode());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sParentValue = getParentValue();
         if(RString.isNotEmpty(sParentValue)){
            config.set(PTY_PARENT_VALUE, sParentValue);
         }
         String sRecordCode = getRecordCode();
         if(RString.isNotEmpty(sRecordCode)){
            config.set(PTY_RECORD_CODE, sRecordCode);
         }
      }else if(EXmlConfig.Default == type){
         String sDataWhere = getDataWhere();
         if(RString.isNotEmpty(sDataWhere)){
