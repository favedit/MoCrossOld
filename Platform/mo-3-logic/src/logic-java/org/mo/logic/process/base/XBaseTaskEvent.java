...
// <T>任务事件对象的XML节点基类。</T>
//============================================================
public abstract class XBaseTaskEvent
      extends FXmlObject
      implements
         XObjectFace
{
   // 名称定义
   public static final String NAME = "TaskEvent";
   ...
   // 执行顺序的名称定义
   public static final String PTY_EXECUTE_ORDER = "execute_order";
   ...
   // 执行顺序的定义
   @AName("execute_order")
   protected String _executeOrder;
   ...
   // @param value 执行顺序
   //============================================================
   public void setExecuteOrder(String value){
      _executeOrder = value;
   }
   ...
   public String innerGet(String name){
      ...
      }else if(PTY_EXECUTE_ORDER.equalsIgnoreCase(name)){
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("execute_order")){
            setExecuteOrder(config.get(PTY_EXECUTE_ORDER));
         }
         ...
         if(RString.isEmpty(getExecuteOrder())){
            if(config.contains("execute_order")){
               setExecuteOrder(config.get(PTY_EXECUTE_ORDER));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getExecuteOrder())){
            config.set(PTY_EXECUTE_ORDER, getExecuteOrder());
         }
         ...
         String sExecuteOrder = getExecuteOrder();
         if(RString.isNotEmpty(sExecuteOrder)){
            config.set(PTY_EXECUTE_ORDER, sExecuteOrder);
         }
