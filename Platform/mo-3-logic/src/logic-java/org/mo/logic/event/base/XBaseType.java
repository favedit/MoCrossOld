         ...
         XObjectFace
{
   ...
   // 服务名称的名称定义
   public static final String PTY_SERVER_LABEL = "server_label";

   // 处理类型的名称定义
   public static final String PTY_PROCESS_CD = "process_cd";
   ...
   // 服务名称的定义
   @AName("server_label")
   protected String _serverLabel;

   // 处理类型的定义
   @AName("process_cd")
   protected String _processCd;
   ...
   public String innerGet(String name){
      if(RString.isEmpty(name)){
         ...
         return getIsValid();
      }else if(PTY_SERVER_LABEL.equalsIgnoreCase(name)){
         return getServerLabel();
      }else if(PTY_PROCESS_CD.equalsIgnoreCase(name)){
         return getProcessCd();
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("server_label")){
            setServerLabel(config.get(PTY_SERVER_LABEL));
         }
         if(config.contains("process_cd")){
            setProcessCd(config.get(PTY_PROCESS_CD));
         }
         ...
         if(RString.isEmpty(getServerLabel())){
            if(config.contains("server_label")){
               setServerLabel(config.get(PTY_SERVER_LABEL));
            }
         }
         if(RString.isEmpty(getProcessCd())){
            if(config.contains("process_cd")){
               setProcessCd(config.get(PTY_PROCESS_CD));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getServerLabel())){
            config.set(PTY_SERVER_LABEL, getServerLabel());
         }
         if(RString.isNotEmpty(getProcessCd())){
            config.set(PTY_PROCESS_CD, getProcessCd());
         }
         ...
         String sServerLabel = getServerLabel();
         if(RString.isNotEmpty(sServerLabel)){
            config.set(PTY_SERVER_LABEL, sServerLabel);
         }
         String sProcessCd = getProcessCd();
         if(RString.isNotEmpty(sProcessCd)){
            config.set(PTY_PROCESS_CD, sProcessCd);
         }
