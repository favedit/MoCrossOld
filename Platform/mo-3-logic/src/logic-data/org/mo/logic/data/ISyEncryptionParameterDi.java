...
/**
 * <T>数据库逻辑包(SY_ENCRYPTION_PARAMETER_DI)的代理对象接口</T>
 *
 * @author MAOCY
 * @version 1.0.1
 */
public interface ISyEncryptionParameterDi
      extends
         ILogicUnit
{
   /**
    * <T>根据参数检索对应的key并将其返回。</T>
    *
    * @param logic 业务逻辑
    * @param params 打包字符串
    * @return 记录对象
    */
   FSqlFunction encode(Object logic,
                       Object params);

   /**
    * <T>根据key检索对应的parmeters并将其返回。</T>
    *
    * @param key 加密键值
    * @return 记录MAP
    */
   FSqlFunction findParametersPack(Object key);
    ...
    * <T>查询当前数据集中是否含有指定的编号。</T>
    *
    * @param encryptionKey 加密键码
    * @return 是否含有指定编号
   @return 对象标识
    */
   FSqlFunction containsIdByEncryptionKey(Object encryptionKey);
   ...
   /**
    * <T>根据加密键码(<C>ENCRYPTION_KEY</C>)获得当前记录的编号</T>
    * <UL>
    * <L>若存在则返回被查询的记录对象的唯一标识。</L>
    * <L>如果记录不存在，则产生记录不存在的例外。</L>
    * </UL>
    ...
    * @return 对象标识
    */
   FSqlFunction getIdByEncryptionKey(Object encryptionKey);
   ...
   /**
    * <T>根据加密键码(<C>ENCRYPTION_KEY</C>)查找当前记录的编号</T>
    *
    * @param encryptionKey 加密键码
    * @param exists 存在标志
    * @return 对象标识
    */
   FSqlFunction findIdByEncryptionKey(Object encryptionKey,
                                      Object exists);
   ...
   /**
    * <T>根据记录的编号查找加密键码(<C>ENCRYPTION_KEY</C>)的数据内容</T>
    *
    * @param objectId 记录的编号
    * @return 加密键码
    */
   FSqlFunction getEncryptionKey(Object objectId);

   /**
    * <T>根据记录的编号查找过期次数(<C>OVERDUE_COUNT</C>)的数据内容</T>
    *
    * @param objectId 记录的编号
    * @return 过期次数
    */
   FSqlFunction getOverdueCount(Object objectId);

   /**
    * <T>根据记录的编号查找过期时间(<C>OVERDUE_DATE</C>)的数据内容</T>
    *
    * @param objectId 记录的编号
    * @return 过期时间
    */
   FSqlFunction getOverdueDate(Object objectId);

   /**
    * <T>根据记录的编号查找访问次数(<C>VISITE_COUNT</C>)的数据内容</T>
    *
    * @param objectId 记录的编号
    * @return 访问次数
    */
   FSqlFunction getVisiteCount(Object objectId);

   /**
    * <T>根据记录的编号查找最后访问时间(<C>VISITE_DATE</C>)的数据内容</T>
    *
    * @param objectId 记录的编号
    * @return 最后访问时间
    */
   FSqlFunction getVisiteDate(Object objectId);

   /**
    * <T>根据记录的编号查找对照参数(<C>PARAMETERS</C>)的数据内容</T>
    *
    * @param objectId 记录的编号
    * @return 对照参数
    */
    ...
    * @param params 打包字符串
    */
   FSqlProcedure register(Object logic,
                          Object key,
                          Object params);
    ...
    * @param key
    */
   FSqlProcedure unregister(Object logic,
                            Object key);
