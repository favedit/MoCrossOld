...
public class SGameProcessInfo{
   ...
   public void parse(String source){
      ...
      index = source.indexOf(' ');
      if(-1 != index){
         memRate = RFloat.parse(source.substring(0, index));
      }
      source = source.substring(index).trim();
      // 读取虚拟内存大小
      index = source.indexOf(' ');
      if(-1 != index){
         memoryVirtual = RLong.parse(source.substring(0, index));
      }
      source = source.substring(index).trim();
      // 读取总共内存大小
      index = source.indexOf(' ');
      if(-1 != index){
         memoryTotal = RLong.parse(source.substring(0, index));
      }
      source = source.substring(index).trim();
      // 读取终端
      index = source.indexOf(' ');
