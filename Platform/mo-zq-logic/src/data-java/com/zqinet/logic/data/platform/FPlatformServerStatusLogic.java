...
// <T>服务器状态逻辑。</T>
//============================================================
public class FPlatformServerStatusLogic extends FLogicDataset
{
   // 的定义。
   public final static FLogicTable Table = new FLogicTable("PF_SERVER_STATUS");
   ...
   // 字段标识的定义。
   public final static FLogicField FieldStatusKey = new FLogicField("STATUS_KEY");

   // 字段参数的定义。
   public final static FLogicField FieldParam = new FLogicField("PARAM");
   ...
   // <T>构造服务器状态逻辑单元。</T>
   //============================================================
   public FPlatformServerStatusLogic(){
   }
   ...
   // @param connection 链接
   //============================================================
   public FPlatformServerStatusLogic(ISqlConnection connection){
   ...
   // @return 数据单元
   //============================================================
   public FPlatformServerStatusUnit find(long recordId){
      // 检查记录编号
      if(0 == recordId){
         return null;
      }
      ...
      // 获得数据
      FPlatformServerStatusUnit unit = new FPlatformServerStatusUnit();
      unit.load(row);
      return unit;
   }
   ...
   // @return 数据单元
   //============================================================
   public FPlatformServerStatusUnit serach(String whereSql){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      sql.append(" WHERE ");
   ...
   // @return 数据单元集合
   //============================================================
   public FPlatformServerStatusUnit[] fetch(String whereSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FPlatformServerStatusUnit[] fetch(String whereSql, String orderSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FPlatformServerStatusUnit[] fetch(String whereSql, String orderSql, int limitCount){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      ...
      // 获得数据
      FPlatformServerStatusUnit[] units = new FPlatformServerStatusUnit[count];
   ...
   // @return 数据单元集合
   //============================================================
   public FPlatformServerStatusUnit[] fetchAll(){
      return fetch(null);
   }
   ...
   // @return 处理结果
   //============================================================
   public boolean doInsert(FPlatformServerStatusUnit unit){
      // 生成命令
      ...
      sql.append(',');
      if(RString.isEmpty(unit.statusKey())){
         sql.append("NULL");
      }else{
         sql.append('\'');
         sql.append(_connection.formatValue(unit.statusKey()));
         sql.append('\'');
      }
      sql.append(',');
      if(RString.isEmpty(unit.param())){
         sql.append("NULL");
      }else{
         sql.append('\'');
         sql.append(_connection.formatValue(unit.param()));
         sql.append('\'');
      }
      sql.append(',');
      if(RString.isEmpty(unit.value())){
         sql.append("NULL");
      }else{
         sql.append('\'');
         sql.append(_connection.formatValue(unit.value()));
         sql.append('\'');
      }
   ...
   // @return 处理结果
   //============================================================
   public boolean doUpdate(FPlatformServerStatusUnit unit, long recordId){
      // 检查记录编号
      if(0 == recordId){
