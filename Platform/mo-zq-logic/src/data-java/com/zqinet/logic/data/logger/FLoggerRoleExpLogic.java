...
// <T>玩家经验改变日志表逻辑。</T>
//============================================================
public class FLoggerRoleExpLogic extends FLogicDataset
{
   // 的定义。
   public final static FLogicTable Table = new FLogicTable("LOG_ROLE_EXP");
   ...
   // 字段经验类型的定义。
   public final static FLogicField FieldExpType = new FLogicField("EXP_TYPE");

   // 字段获取经验的类型的定义。
   public final static FLogicField FieldOpType = new FLogicField("OP_TYPE");

   // 字段当前经验(改变后的)的定义。
   public final static FLogicField FieldCurExp = new FLogicField("CUR_EXP");

   // 字段改变的经验数量的定义。
   public final static FLogicField FieldDeltaExp = new FLogicField("DELTA_EXP");
   ...
   // @param connection 链接
   //============================================================
   public FLoggerRoleExpLogic(ISqlConnection connection){
   ...
   // @return 数据单元
   //============================================================
   public FLoggerRoleExpUnit find(long recordId){
      // 检查记录编号
      if(0 == recordId){
         return null;
      }
      ...
      // 获得数据
      FLoggerRoleExpUnit unit = new FLoggerRoleExpUnit();
      unit.load(row);
      return unit;
   }
   ...
   // @return 数据单元
   //============================================================
   public FLoggerRoleExpUnit serach(String whereSql){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      sql.append(" WHERE ");
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleExpUnit[] fetch(String whereSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleExpUnit[] fetch(String whereSql, String orderSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleExpUnit[] fetch(String whereSql, String orderSql, int limitCount){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      ...
      // 获得数据
      FLoggerRoleExpUnit[] units = new FLoggerRoleExpUnit[count];
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleExpUnit[] fetchAll(){
      return fetch(null);
   }
   ...
   // @return 处理结果
   //============================================================
   public boolean doInsert(FLoggerRoleExpUnit unit){
      // 生成命令
      ...
      sql.append(',');
      sql.append(unit.expType());
      sql.append(',');
      sql.append(unit.opType());
      sql.append(',');
      sql.append(unit.curExp());
      sql.append(',');
      sql.append(unit.deltaExp());
      sql.append(',');
   ...
   // @return 处理结果
   //============================================================
   public boolean doUpdate(FLoggerRoleExpUnit unit, long recordId){
      // 检查记录编号
      if(0 == recordId){
      ...
      if(unit.isCurExpChanged()){
         sql.append(",`CUR_EXP`=");
         sql.append(unit.curExp());
      }
      if(unit.isDeltaExpChanged()){
         sql.append(",`DELTA_EXP`=");
         sql.append(unit.deltaExp());
      }
