...
// <T>日志分析消费统计逻辑。</T>
//============================================================
public class FAnalysisServiceOrderLogic extends FLogicDataset
{
   // 的定义。
   public final static FLogicTable Table = new FLogicTable("AS_SVC_ORDER");
   ...
   // 字段每日新增充值人数的定义。
   public final static FLogicField FieldNewRoleCount = new FLogicField("NEW_ROLE_COUNT");

   // 字段充值人次的定义。
   public final static FLogicField FieldConsumeCount = new FLogicField("CONSUME_COUNT");

   // 字段领取礼包个数的定义。
   public final static FLogicField FieldReceiveGiftNum = new FLogicField("RECEIVE_GIFT_NUM");
   ...
   // <T>构造日志分析消费统计逻辑单元。</T>
   //============================================================
   public FAnalysisServiceOrderLogic(){
   }
   ...
   // @param connection 链接
   //============================================================
   public FAnalysisServiceOrderLogic(ISqlConnection connection){
   ...
   // @return 数据单元
   //============================================================
   public FAnalysisServiceOrderUnit find(long recordId){
      // 检查记录编号
      if(0 == recordId){
         return null;
      }
      ...
      // 获得数据
      FAnalysisServiceOrderUnit unit = new FAnalysisServiceOrderUnit();
      unit.load(row);
      return unit;
   }
   ...
   // @return 数据单元
   //============================================================
   public FAnalysisServiceOrderUnit serach(String whereSql){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      sql.append(" WHERE ");
   ...
   // @return 数据单元集合
   //============================================================
   public FAnalysisServiceOrderUnit[] fetch(String whereSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FAnalysisServiceOrderUnit[] fetch(String whereSql, String orderSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FAnalysisServiceOrderUnit[] fetch(String whereSql, String orderSql, int limitCount){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      ...
      // 获得数据
      FAnalysisServiceOrderUnit[] units = new FAnalysisServiceOrderUnit[count];
   ...
   // @return 数据单元集合
   //============================================================
   public FAnalysisServiceOrderUnit[] fetchAll(){
      return fetch(null);
   }
   ...
   // @return 处理结果
   //============================================================
   public boolean doInsert(FAnalysisServiceOrderUnit unit){
      // 生成命令
      ...
      sql.append(",`NEW_ROLE_COUNT`");
      sql.append(",`CONSUME_COUNT`");
      sql.append(",`RECEIVE_GIFT_NUM`");
      ...
      sql.append(',');
      sql.append(unit.newRoleCount());
      sql.append(',');
      sql.append(unit.consumeCount());
      sql.append(',');
      sql.append(unit.receiveGiftNum());
   ...
   // @return 处理结果
   //============================================================
   public boolean doUpdate(FAnalysisServiceOrderUnit unit, long recordId){
      // 检查记录编号
      if(0 == recordId){
      ...
      if(unit.isNewRoleCountChanged()){
         sql.append(",`NEW_ROLE_COUNT`=");
         sql.append(unit.newRoleCount());
      }
      if(unit.isConsumeCountChanged()){
         sql.append(",`CONSUME_COUNT`=");
         sql.append(unit.consumeCount());
      }
      if(unit.isReceiveGiftNumChanged()){
         sql.append(",`RECEIVE_GIFT_NUM`=");
         sql.append(unit.receiveGiftNum());
      }
