...
// <T>角色名逻辑。</T>
//============================================================
public class FGameRoleNameLogic extends FLogicDataset
{
   // 的定义。
   public final static FLogicTable Table = new FLogicTable("GM_ROLE_NAME");
   ...
   // 字段名字的定义。
   public final static FLogicField FieldFullName = new FLogicField("FULL_NAME");

   // 字段使用的定义。
   public final static FLogicField FieldUserTypeCd = new FLogicField("USER_TYPE_CD");
   ...
   // @param connection 链接
   //============================================================
   public FGameRoleNameLogic(ISqlConnection connection){
   ...
   // @return 数据单元
   //============================================================
   public FGameRoleNameUnit find(long recordId){
      // 检查记录编号
      if(0 == recordId){
         return null;
      }
      ...
      // 获得数据
      FGameRoleNameUnit unit = new FGameRoleNameUnit();
      unit.load(row);
      return unit;
   }
   ...
   // @return 数据单元
   //============================================================
   public FGameRoleNameUnit serach(String whereSql){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      sql.append(" WHERE ");
   ...
   // @return 数据单元集合
   //============================================================
   public FGameRoleNameUnit[] fetch(String whereSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FGameRoleNameUnit[] fetch(String whereSql, String orderSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FGameRoleNameUnit[] fetch(String whereSql, String orderSql, int limitCount){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      ...
      // 获得数据
      FGameRoleNameUnit[] units = new FGameRoleNameUnit[count];
   ...
   // @return 数据单元集合
   //============================================================
   public FGameRoleNameUnit[] fetchAll(){
      return fetch(null);
   }
   ...
   // @return 处理结果
   //============================================================
   public boolean doInsert(FGameRoleNameUnit unit){
      // 生成命令
      ...
      sql.append(',');
      if(RString.isEmpty(unit.fullName())){
         sql.append("NULL");
      }else{
         sql.append('\'');
         sql.append(_connection.formatValue(unit.fullName()));
         sql.append('\'');
      }
      sql.append(',');
      sql.append(unit.userTypeCd());
      sql.append(',');
   ...
   // @return 处理结果
   //============================================================
   public boolean doUpdate(FGameRoleNameUnit unit, long recordId){
      // 检查记录编号
      if(0 == recordId){
