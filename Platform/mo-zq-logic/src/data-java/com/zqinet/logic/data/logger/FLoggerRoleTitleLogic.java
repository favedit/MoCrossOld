...
// <T>角色称号改变日志逻辑。</T>
//============================================================
public class FLoggerRoleTitleLogic extends FLogicDataset
{
   // 的定义。
   public final static FLogicTable Table = new FLogicTable("LOG_ROLE_TITLE");
   ...
   // 字段称号变化事件的定义。
   public final static FLogicField FieldTitleEvent = new FLogicField("TITLE_EVENT");

   // 字段称号编号的定义。
   public final static FLogicField FieldTitleId = new FLogicField("TITLE_ID");
   ...
   // @param connection 链接
   //============================================================
   public FLoggerRoleTitleLogic(ISqlConnection connection){
   ...
   // @return 数据单元
   //============================================================
   public FLoggerRoleTitleUnit find(long recordId){
      // 检查记录编号
      if(0 == recordId){
         return null;
      }
      ...
      // 获得数据
      FLoggerRoleTitleUnit unit = new FLoggerRoleTitleUnit();
      unit.load(row);
      return unit;
   }
   ...
   // @return 数据单元
   //============================================================
   public FLoggerRoleTitleUnit serach(String whereSql){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      sql.append(" WHERE ");
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleTitleUnit[] fetch(String whereSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleTitleUnit[] fetch(String whereSql, String orderSql){
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleTitleUnit[] fetch(String whereSql, String orderSql, int limitCount){
      // 生成命令
      FSql sql = new FSql("SELECT * FROM ");
      sql.append(Table.name());
      ...
      // 获得数据
      FLoggerRoleTitleUnit[] units = new FLoggerRoleTitleUnit[count];
   ...
   // @return 数据单元集合
   //============================================================
   public FLoggerRoleTitleUnit[] fetchAll(){
      return fetch(null);
   }
   ...
   // @return 处理结果
   //============================================================
   public boolean doInsert(FLoggerRoleTitleUnit unit){
      // 生成命令
      ...
      sql.append(',');
      sql.append(unit.titleEvent());
      sql.append(',');
      sql.append(unit.titleId());
   ...
   // @return 处理结果
   //============================================================
   public boolean doUpdate(FLoggerRoleTitleUnit unit, long recordId){
      // 检查记录编号
      if(0 == recordId){
      ...
      if(unit.isTitleEventChanged()){
         sql.append(",`TITLE_EVENT`=");
         sql.append(unit.titleEvent());
      }
      if(unit.isTitleIdChanged()){
         sql.append(",`TITLE_ID`=");
         sql.append(unit.titleId());
      }
