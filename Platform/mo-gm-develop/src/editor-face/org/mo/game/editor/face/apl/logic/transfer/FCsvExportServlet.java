...
public class FCsvExportServlet
      implements
         ICsvExportServlet
{
   ...
   public void process(IWebContext context,
      ...
      try{
         ...
         if(RString.isNotEmpty(datasetName) && RString.isNotEmpty(transferName)){
            // 输出前，需要判断是否要输出。
            if(_logger.debugAble()){
               _logger.debug(this, "build", "Get transfer name. (transfer={0}, dataset={1})", transferName, datasetName);
            }
   ...
   protected String writeCsv(IWebContext context,
      ...
      try{
         if(dataset != null){
            ...
            // 输出前，需要判断是否要输出。
            if(_logger.debugAble()){
               _logger.debug(this, "writeCsv", "Excute sql={0}", newSql);
            }
            // 打开路径格式为GB18030
