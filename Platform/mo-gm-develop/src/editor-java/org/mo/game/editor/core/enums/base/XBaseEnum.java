...
// <T>枚举对象的XML节点基类。</T>
//============================================================
public abstract class XBaseEnum
      extends FXmlObject
      implements
         XObjectFace,
         XObjectSource
{
   ...
   // 计数器的名称定义
   public static final String PTY_IS_COUNT = "is_count";

   // 枚举类型的名称定义
   public static final String PTY_ENUM_DATA_TYPE_CD = "enum_data_type_cd";

   // 集合的名称定义
   public static final String PTY_IS_SET = "is_set";

   // 集合类型的名称定义
   public static final String PTY_SET_DATA_TYPE_CD = "set_data_type_cd";

   // 代码的名称定义
   public static final String PTY_IS_CODE = "is_code";

   // 参考值的名称定义
   public static final String PTY_REFER_VALUE = "refer_value";
   ...
   // 计数器的定义
   @AName("is_count")
   protected String _isCount;

   // 枚举类型的定义
   @AName("enum_data_type_cd")
   protected String _enumDataTypeCd;
   ...
   // 集合类型的定义
   @AName("set_data_type_cd")
   protected String _setDataTypeCd;

   // 代码的定义
   @AName("is_code")
   protected String _isCode;

   // 参考值的定义
   @AName("refer_value")
   protected String _referValue;
   ...
   // @param value 计数器
   //============================================================
   public void setIsCount(String value){
      _isCount = value;
   }
   ...
   // @return 枚举类型
   //============================================================
   public String getEnumDataTypeCd(){
      return _enumDataTypeCd;
   }
   ...
   // @param value 枚举类型
   //============================================================
   public void setEnumDataTypeCd(String value){
      _enumDataTypeCd = value;
   }
   ...
   // @return 集合类型
   //============================================================
   public String getSetDataTypeCd(){
      return _setDataTypeCd;
   }
   ...
   // @param value 集合类型
   //============================================================
   public void setSetDataTypeCd(String value){
      _setDataTypeCd = value;
   }
   ...
   public String innerGet(String name){
      ...
      }else if(PTY_IS_COUNT.equalsIgnoreCase(name)){
         return getIsCount();
      }else if(PTY_ENUM_DATA_TYPE_CD.equalsIgnoreCase(name)){
         return getEnumDataTypeCd();
      }else if(PTY_IS_SET.equalsIgnoreCase(name)){
         return getIsSet();
      }else if(PTY_SET_DATA_TYPE_CD.equalsIgnoreCase(name)){
         return getSetDataTypeCd();
      }else if(PTY_IS_CODE.equalsIgnoreCase(name)){
         return getIsCode();
      }else if(PTY_REFER_VALUE.equalsIgnoreCase(name)){
         return getReferValue();
      }
      return null;
   }
   ...
   public void innerSet(String name, String value){
      if(RString.isEmpty(name)){
         ...
         setEnumDataTypeCd(value);
      }else if(PTY_IS_SET.equalsIgnoreCase(name)){
         setIsSet(value);
      }else if(PTY_SET_DATA_TYPE_CD.equalsIgnoreCase(name)){
         setSetDataTypeCd(value);
      }else if(PTY_IS_CODE.equalsIgnoreCase(name)){
         setIsCode(value);
      }else if(PTY_REFER_VALUE.equalsIgnoreCase(name)){
         setReferValue(value);
   ...
   public void loadConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(config.contains("is_count")){
            setIsCount(config.get(PTY_IS_COUNT));
         }
         if(config.contains("enum_data_type_cd")){
            setEnumDataTypeCd(config.get(PTY_ENUM_DATA_TYPE_CD));
         }
         if(config.contains("is_set")){
            setIsSet(config.get(PTY_IS_SET));
         }
         if(config.contains("set_data_type_cd")){
            setSetDataTypeCd(config.get(PTY_SET_DATA_TYPE_CD));
         }
         if(config.contains("is_code")){
            setIsCode(config.get(PTY_IS_CODE));
         }
         if(config.contains("refer_value")){
            setReferValue(config.get(PTY_REFER_VALUE));
         }
         ...
         if(RString.isEmpty(getIsCount())){
            if(config.contains("is_count")){
               setIsCount(config.get(PTY_IS_COUNT));
            }
         }
         if(RString.isEmpty(getEnumDataTypeCd())){
            if(config.contains("enum_data_type_cd")){
               setEnumDataTypeCd(config.get(PTY_ENUM_DATA_TYPE_CD));
            }
         }
         ...
         if(RString.isEmpty(getSetDataTypeCd())){
            if(config.contains("set_data_type_cd")){
               setSetDataTypeCd(config.get(PTY_SET_DATA_TYPE_CD));
            }
         }
         if(RString.isEmpty(getIsCode())){
            if(config.contains("is_code")){
               setIsCode(config.get(PTY_IS_CODE));
            }
         }
         if(RString.isEmpty(getReferValue())){
            if(config.contains("refer_value")){
               setReferValue(config.get(PTY_REFER_VALUE));
            }
         }
   ...
   public void saveConfig(FXmlNode config, EXmlConfig type){
      ...
      if(EXmlConfig.Full == type){
         ...
         if(RString.isNotEmpty(getIsCount())){
            config.set(PTY_IS_COUNT, getIsCount());
         }
         if(RString.isNotEmpty(getEnumDataTypeCd())){
            config.set(PTY_ENUM_DATA_TYPE_CD, getEnumDataTypeCd());
         }
         if(RString.isNotEmpty(getIsSet())){
            config.set(PTY_IS_SET, getIsSet());
         }
         if(RString.isNotEmpty(getSetDataTypeCd())){
            config.set(PTY_SET_DATA_TYPE_CD, getSetDataTypeCd());
         }
         if(RString.isNotEmpty(getIsCode())){
            config.set(PTY_IS_CODE, getIsCode());
         }
         if(RString.isNotEmpty(getReferValue())){
            config.set(PTY_REFER_VALUE, getReferValue());
         }
      }else if(EXmlConfig.Simple == type){
         if(RString.isNotEmpty(getName())){
            config.set(PTY_NAME, getName());
         ...
         String sIsCount = getIsCount();
         if(RString.isNotEmpty(sIsCount)){
            config.set(PTY_IS_COUNT, sIsCount);
         }
         String sEnumDataTypeCd = getEnumDataTypeCd();
         if(RString.isNotEmpty(sEnumDataTypeCd)){
            config.set(PTY_ENUM_DATA_TYPE_CD, sEnumDataTypeCd);
         }
         String sIsSet = getIsSet();
         if(RString.isNotEmpty(sIsSet)){
            config.set(PTY_IS_SET, sIsSet);
         }
         String sSetDataTypeCd = getSetDataTypeCd();
         if(RString.isNotEmpty(sSetDataTypeCd)){
            config.set(PTY_SET_DATA_TYPE_CD, sSetDataTypeCd);
         }
         String sIsCode = getIsCode();
         if(RString.isNotEmpty(sIsCode)){
            config.set(PTY_IS_CODE, sIsCode);
         }
         String sReferValue = getReferValue();
         if(RString.isNotEmpty(sReferValue)){
            config.set(PTY_REFER_VALUE, sReferValue);
         }
      }else if(EXmlConfig.Default == type){
         String sIsServer = getIsServer();
         if(RString.isNotEmpty(sIsServer)){
