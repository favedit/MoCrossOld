...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/logic/process/Process.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='业务-流程列表' icon='sys.pst.mgr' target='frmMain' page='list@#/logic/process/Process.wa?form_name=system.logic.process.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.logic.process'/>
<!-- Body begin ---------------------------------------------->
