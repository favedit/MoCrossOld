...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/logic/property/Property.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='业务-属性分组' icon='sys.pst.mgr' target='frmMain' page='list@#/logic/property/Property.wa?form_name=system.logic.property.GroupList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.logic.property'/>
<!-- Body begin ---------------------------------------------->
