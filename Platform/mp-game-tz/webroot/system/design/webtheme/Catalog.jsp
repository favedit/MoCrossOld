...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/webtheme/WebTheme.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='主题列表' icon='sys.pst.mgr' target='frmMain' page='#/design/webtheme/WebTheme.wa?do=list&amp;form_name=design.webtheme.WebThemeList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.design.webtheme'/>
<!-- Body begin ---------------------------------------------->
