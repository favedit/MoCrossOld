...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/webtree/WebTree.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='树表定义' icon='sys.pst.mgr' target='frmMain' page='#/design/webtree/WebTree.wa?do=list&amp;form_name=design.webtree.TreeViewList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>

<jc:tree name='xTree' source='system.design.webtree'/>
