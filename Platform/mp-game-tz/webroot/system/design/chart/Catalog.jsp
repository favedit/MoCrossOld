...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/chart/Chart.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='图表设计列表' icon='sys.pst.mgr' target='frmMain' page='#/design/webtools/WebTools.wa?do=list&amp;form_name=design.chart.ChartList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.design.chart'/>
<!-- Body begin ---------------------------------------------->
