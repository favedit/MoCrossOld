...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/design/sidebar/SideBar.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' align='right'>
	<ToolButtonText name='btnForm' label='分割栏列表' icon='sys.pst.mgr' target='frmMain' page='#/design/webtools/WebTools.wa?do=list&amp;form_name=design.sidebar.SideBarList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.design.sidebar'/>
<!-- Body begin ---------------------------------------------->
