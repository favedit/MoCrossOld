...
function onNodeClick(sender, node){
	...
	fmMain.action = '<jh:context/>/system/help/Help.wa?do=update';
	fmMain.submit();
}
function _onloadAll(){
	MoJS.connect();
...
<ToolBar width='100%' height='100%' align='right'>
	<ToolButtonText name='btnForm' label='帮助列表' icon='sys.pst.mgr' target='frmMain' page='#/system/help/Help.wa?do=list&amp;form_name=system.help.HelpList'/>
	<ToolButton name='btnRefresh' icon='tool.refresh' action='refreshNode()'/>
</ToolBar>
</XML>
<!-- TreeView config ----------------------------------------->
<jc:tree name='xTree' source='system.system.help'/>
</XML>
<!-- Body begin ---------------------------------------------->
