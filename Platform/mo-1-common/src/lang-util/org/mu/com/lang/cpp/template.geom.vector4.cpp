...
// @return 是否解析成功
//============================================================
TBool S{type_name}Vector4::Parse(TCharC* pValue){
   // 检查长度
   ...
   w = R{type_name}::Parse(temp.MemoryC());
   return ETrue;
}
...
// @return 字符串
//============================================================
TCharC* S{type_name}Vector4::Format(TChar* pBuffer, TInt capacity){
   TStringRefer result(pBuffer, capacity);
   result.AssignFormat(TC("x={format}, y={format}, z={format}, w={format}"), x, y, z, w);
   return pBuffer;
}
...
// @return 字符串
//============================================================
TCharC* S{type_name}Vector4::ToString(TChar* pBuffer, TInt capacity){
   TStringRefer result(pBuffer, capacity);
   result.AppendFormat(TC("{format},{format},{format},{format}"), x, y, z, w);
   return pBuffer;
}
