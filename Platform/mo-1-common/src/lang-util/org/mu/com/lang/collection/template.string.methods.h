public:
   //------------------------------------------------------------
   // <T>获得数据只读指针</T>
   MO_INLINE( operator const {char}*() const ){
      return {memory};
   }
   //------------------------------------------------------------
   // <T>获得数据指针。</T>
   ...
   MO_INLINE( TBool operator<(const MString{type}& value) const ){
      return RTypes<{char}>::Compare({memory}, _length, value.MemoryC(), value.Length()) < 0;
   }
   //------------------------------------------------------------
...
public:
   ...
   MO_INLINE( TIteratorC IteratorC() ){
      return TIteratorC(_length, {memory});
   }
   //------------------------------------------------------------
   // <T>获取迭代器。</T>
   MO_INLINE( TIterator Iterator() ){
      return TIterator(&_length, {memory});
   }
   //------------------------------------------------------------
   // <T>获取指定位置开始的迭代器。</T>
   MO_INLINE( TIterator Iterator(TInt index) ){
      MO_ASSERT_RANGE(index, 0, _length);
      return TIterator(&_length, {memory}, index);
   }
   //------------------------------------------------------------
   // <T>获取当前数组的指针对象。</T>
   MO_INLINE( TString{type}PtrC PtrC() ){
      return TString{type}PtrC(_length, {memory});
   }
   //------------------------------------------------------------
   // <T>获取当前数组的指针对象。</T>
   MO_INLINE( TString{type}Ptr Ptr() ){
      return TString{type}Ptr({capacity}, &_length, {memory});
   }
   //------------------------------------------------------------
   // <T>获得末尾为0的指针。</T>
   ...
   MO_INLINE( THashCode HashCode() const ){
      return RTypes<{char}>::MakeHashCode({memory}, _length);
   }
public:
   //------------------------------------------------------------
   // <T>当前字符串和指定字符串是否相等。</T>
