...
// <T>ANSI字符串工具类。</T>
//============================================================
class MO_CM_DECLARE RString{type}{
public:
   static {char} EmptyPtr[1];
public:
   static TInt Length({char}C* pValue);
   static TBool Equals({char}C* pSource, {char}C* pTarget);
   static THashCode MakeHashCode({char}C* pSource);
   static THashCode MakeHashCodeIgnoreCase({char}C* pValues);
public:
...
public:
   static TInt Replace({char}* pValues, {char}* pSource, {char}* pTarget, {char}* pBuffer, TInt bufferLength);
{source_tools}
};
...
// <T>只读字符串指针。</T>
//============================================================
class TString{type}PtrC : public TPtrC<{char}>{
public:
   //------------------------------------------------------------
   // <T>构造只读字符串指针。</T>
   TString{type}PtrC(const {char}* pMemory){
      Assign(pMemory);
   }
   //------------------------------------------------------------
   // <T>构造只读字符串指针。</T>
   TString{type}PtrC(TInt length, const {char}* pMemory) : TPtrC<{char}>(length, pMemory){
   }
   //------------------------------------------------------------
   // <T>构造只读字符串指针。</T>
   TString{type}PtrC(const TPtrC<{char}>& ptr) : TPtrC<{char}>(ptr){
   }
   //------------------------------------------------------------
   // <T>构造只读字符串指针。</T>
   TString{type}PtrC(const TString{type}PtrC& ptr) : TPtrC<{char}>(ptr){
   }
public:
   //------------------------------------------------------------
   // <T>重载指针赋值操作。</T>
   void operator=(const {char}* pMemory){
      Assign(pMemory);
   }
public:
   //------------------------------------------------------------
   // <T>接收一个字符串内容。</T>
   void Assign(const {char}* pMemory, TInt length = -1){
      MO_ASSERT(pMemory);
      if(length < 0){
         length = RChar{type}::Length(pMemory);
      }
      Set(length, pMemory);
   }
};
...
// <T>字符串指针。</T>
//============================================================
class TString{type}Ptr : public TPtr<{char}>{
public:
   //------------------------------------------------------------
   // <T>构造字符串指针。</T>
   TString{type}Ptr(){
   ...
   //------------------------------------------------------------
   // <T>构造字符串指针。</T>
   TString{type}Ptr(TInt capacity, TInt* pLength, {char}* pMemory) : TPtr<{char}>(capacity, pLength, pMemory){
   }
   //------------------------------------------------------------
   // <T>构造字符串指针。</T>
   TString{type}Ptr(const TPtr<{char}>& ptr) : TPtr<{char}>(ptr){
   }
   //------------------------------------------------------------
   // <T>构造字符串指针。</T>
   TString{type}Ptr(const TString{type}Ptr& ptr) : TPtr<{char}>(ptr){
   }
};
...
// <T>字符串迭代器定义。</T>
//============================================================
typedef TArrayIteratorC<{char}> TString{type}IteratorC;
typedef TArrayIterator<{char}> TString{type}Iterator;
...
//============================================================
class MO_CM_DECLARE MString{type}{
public:
   // @type 比较函数
...
protected:
   ...
   MO_INLINE( {char}* InnerAlloc(TInt size) ){
      return ({char}*)MO_MEM_ALLOC(sizeof({char}) * size);
   }
   //------------------------------------------------------------
   // <T>析构字符串基类。</T>
...
public:
   //------------------------------------------------------------
   // <T>计算内存扩展容量。</T>
   MO_INLINE( TInt CalculateCapacity(TInt size) ){
      // 当内存不足时，扩大1.5倍内存，但收集量不得小于默认值<C>MO_MEMORY_CAPACITY</C>的内存。
      TInt capacity = MO_LG_MAX(MO_MEMORY_CAPACITY, size);
      if(NULL != _pMemory){
         capacity += capacity >> 1;
      }
      return capacity;
   ...
   void EnsureSize(TInt size){
      if(size > _capacity){
         ...
         // 如果收集不成功，则不进行复制数据处理
         {char}* pAlloc = InnerAlloc(capacity);
         MO_ASSERT(pAlloc);
         // 如果以前内存有数据，则复制以前内存内容
         if(NULL != _pMemory){
            InnerFree(_pMemory);
   ...
   void EnsureResize(TInt size){
      if(size > _capacity){
         ...
         if(NULL != _pMemory){
            if(_length > 0){
               RTypes<{char}>::CopyFast(pAlloc, _pMemory, _length);
            }
            InnerFree(_pMemory);
         }
         // 设置新的内存
...
public:
   ...
   void Split({char}C* pSource, {char}C* pSplitter){
      TInt lengthSource = RChar{type}::Length(pSource);
      TInt lengthSplitter = RChar{type}::Length(pSplitter);
      TInt begin = 0;
      TInt index = 0;
      TInt count = 0;
