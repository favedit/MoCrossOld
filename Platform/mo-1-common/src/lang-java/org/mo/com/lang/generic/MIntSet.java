...
// <T>整数集合。</T>
//============================================================
public abstract class MIntSet<V>
      extends FObject
      implements
         IObjects<V>
{
   ...
   // 哈希集合
   protected SHashIntEntry[] _table;
   ...
   protected void innerResize(){
      ...
      SHashIntEntry entry = null;
      SHashIntEntry next = null;
      SHashIntEntry[] memory = new SHashIntEntry[length << 2];
      // 重置链表
   ...
   public void ensureSize(int count){
      if(null == _values){
         ...
         _table = new SHashIntEntry[CAPACITY];
   ...
   // @param values 数据集合
   //============================================================
   public void assign(MIntSet<V> values){
      ...
      if(_count > 0){
         ...
         for(int n = 0; n < length; n++){
            ...
            while(entry != null){
               ...
               RDump.dump(info, Integer.toString(entry.hash), value);
               entry = entry.next;
            }
         }
         info.append("}");
