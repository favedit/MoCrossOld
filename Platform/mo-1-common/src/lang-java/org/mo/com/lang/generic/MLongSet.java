...
// <T>整数集合。</T>
//============================================================
public abstract class MLongSet<V>
      extends FObject
      implements
         IObjects<V>
{
   ...
   // 哈希集合
   protected SHashLongEntry[] _table;
   ...
   protected void innerResize(){
      ...
      SHashLongEntry entry = null;
      SHashLongEntry next = null;
      SHashLongEntry[] memory = new SHashLongEntry[length << 2];
      // 重置链表
   ...
   public int indexOf(V value){
      ...
      SHashLongEntry entry = _table[hash % _table.length];
      while(null != entry){
   ...
   public void ensureSize(int count){
      if(null == _values){
         ...
         _table = new SHashLongEntry[CAPACITY];
   ...
   // @param values 数据集合
   //============================================================
   public void assign(MLongSet<V> values){
   ...
   public void push(V value){
      ...
      SHashLongEntry entry = _table[index];
      while(null != entry){
         if(entry.hash == hash && value.equals(_values[entry.index])){
      ...
      _table[index] = new SHashLongEntry(hash, _count, _table[index]);
   ...
   public V remove(V value){
      ...
      SHashLongEntry prior = _table[pos];
      SHashLongEntry entry = prior;
      while(null != entry){
         if(entry.hash == hash && value.equals(_values[entry.index])){
            index = entry.index;
            if(entry == prior){
      ...
      if(_count > 0){
         ...
         for(int n = 0; n < length; n++){
            ...
            while(entry != null){
               ...
               RDump.dump(info, Long.toString(entry.hash), value);
               entry = entry.next;
            }
         }
         info.append("}");
