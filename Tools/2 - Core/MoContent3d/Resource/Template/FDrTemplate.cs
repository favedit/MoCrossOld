...
namespace MO.Content3d.Resource.Template
{
   ...
   // <T>模板定义。</T>
   //============================================================
   public class FDrTemplate : FDrResource, IDisposable
   {
      // 日志输出接口
      private static ILogger _logger = RLogger.Find(typeof(FDrTemplate));
      ...
      public override void Export(ERsExportMode modeCd) {
         ...
         // 释放资源
         Dispose();
         _logger.Debug(this, "Export", "Export template success. (file_name={0})", exportFileName);
      }
