...
namespace MS.Common.Lang
{
   ...
   {
      // 初始化容量
      public const int CAPABILITY = 16;
      ...
      public int Find(T[] memory) {
         return RTypes<T>.Find(_memory, 0, _length, memory, 0, memory.Length);
      }
      ...
      public int Find(T[] memory, int offset, int length) {
         return RTypes<T>.Find(_memory, 0, _length, memory, offset, length);
      }
      ...
      public int Find(FTypes<T> memory) {
         return RTypes<T>.Find(_memory, 0, _length, memory.Memory, 0, memory.Length);
      }
      ...
      public void EnsureSize(int size) {
         if(_memory == null) {
            // 创建内存
            int length = Math.Max(size, CAPABILITY);
            _memory = new T[length];
         } else if(size > _memory.Length) {
            // 计算新缓冲内存的大小
            int length = size + _memory.Length;
            // 创建新缓冲内存
      ...
      // @param length 类型个数
      //============================================================
      public bool Append(T[] memory, int offset, int length) {
         if(memory == null) {
            return false;
         }
         if(length <= 0) {
      ...
      //============================================================
      // <T>移除从指定位置开始指定长度的数据。</T>
      //
      // @param offset 位置
      // @param length 长度
      // @return 处理结果
      //============================================================
      public virtual bool Erase(int offset, int length) {
         ...
         int copy = _length - offset - length;
         if(copy > 0) {
            Array.Copy(_memory, offset + length, _memory, offset, copy);
         }
         _length -= length;
         return true;
      }
