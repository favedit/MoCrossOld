...
MO_CLASS_IMPLEMENT_INHERITS(FPd11RenderTarget, FRenderTarget);

//============================================================
// <T>构造渲染目标。</T>
//============================================================
FPd11RenderTarget::FPd11RenderTarget(){
   MO_CLEAR(_piRenderTarget);
   MO_CLEAR(_piDepthStencil);
}
...
//============================================================
FPd11RenderTarget::~FPd11RenderTarget(){
   MO_RELEASE(_piRenderTarget);
   MO_RELEASE(_piDepthStencil);
}
...
// @return 处理结果
//============================================================
TResult FPd11RenderTarget::OnSetup(){
   TResult resultCd = ESuccess;
   MO_CHECK(_pDevice, return ENull);
   FPd11RenderDevice* pDevice = _pDevice->Convert<FPd11RenderDevice>();
   //............................................................
   // 创建渲染目标
   //ID3D11Resource* pBackBuffer = NULL;
   //dxResult = pDevice->NativeSwapChain()->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&pBackBuffer);
   //if(FAILED(dxResult)){
   //   MO_FATAL("Get backbugger failure.");
   //   return EFailure;
   //}
   //ID3D11RenderTargetView* _pRenderTarget = NULL;
   //dxResult = _piDevice->CreateRenderTargetView(pBackBuffer, NULL, &_pRenderTarget);
   //if(FAILED(dxResult)){
...
// @return 处理结果
//============================================================
TResult FPd11RenderTarget::Suspend(){
   TResult resultCd = ESuccess;
   //if(_depthBufferId != 0){
...
// @return 处理结果
//============================================================
TResult FPd11RenderTarget::Resume(){
   OnSetup();
...
// @return 处理结果
//============================================================
TResult FPd11RenderTarget::Dispose(){
   return ESuccess;
}
