...
#define MO_VAL_SHARE_KEY "share_key"
...
TResult FNetTransferService::LoadConfig(FXmlNode* pConfig){
   ...
   while(iterator.Next()){
      ...
      if(pNode->IsName("Property")){
         // 读取属性
         if(pNode->IsAttribute(MO_PTR_NAME, "mode")){
            // 读取工作方式
            TCharC* pMode = pNode->Text();
            if(RString::Equals("multipool", pMode)){
               MO_DEBUG("Load module(%s) property. (mode=%s)", (TCharC*)_name, "multipool");
               _config.multiMode = ETrue;
            }
         }else if(pNode->IsAttribute(MO_PTR_NAME, "protocol")){
            // 读取工作方式
            TCharC* pProtocol = pNode->Text();
            if(RString::Equals("message", pProtocol)){
               MO_DEBUG("Load module(%s) property. (protocol=%s)", (TCharC*)_name, "message");
               _config.protocol = ENetProtocol_Message;
            }else if(RString::Equals("router", pProtocol)){
               MO_DEBUG("Load module(%s) property. (protocol=%s)", (TCharC*)_name, "router");
               _config.protocol = ENetProtocol_Router;
            }
         }else if(pNode->IsAttribute(MO_PTR_NAME, "link_type")){
            // 读取共享锁关联方式
            TCharC* pLinkType = pNode->Text();
            if(RString::Equals("create", pLinkType)){
               MO_DEBUG("Load module(%s) property. (link_type=%s)", (TCharC*)_name, "create");
            }else if(RString::Equals("connect", pLinkType)){
               MO_DEBUG("Load module(%s) property. (link_type=%s)", (TCharC*)_name, "connect");
            }
         }else if(pNode->IsAttribute(MO_PTR_NAME, "data_check")){
            // 读取传输检查方式
            _config.dataCheck = pNode->TextAsBool();
            MO_DEBUG("Load module(%s) property. (data_check=%d)", (TCharC*)_name, _config.dataCheck);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "data_compress")){
            // 读取传输压缩方式
            _config.dataCompress = pNode->TextAsBool();
            MO_DEBUG("Load module(%s) property. (data_compress=%d)", (TCharC*)_name, _config.dataCompress);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "data_mask")){
            // 读取传输加密方式
            _config.dataMask = pNode->TextAsBool();
            MO_DEBUG("Load module(%s) property. (data_mask=%d)", (TCharC*)_name, _config.dataMask);
         }else if(pNode->IsAttribute(MO_PTR_NAME, MO_VAL_SHARE_KEY)){
            // 读取共享锁文件路径
            pNode->GetText(_config.shareKeyFile, MO_FS_FILENAME_LENGTH);
            MO_DEBUG("Load module(%s) property. (share_key=%s)", (TCharC*)_name, _config.shareKeyFile);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "udp_support")){
            // 读取是否支持广播
            _config.udpSupport = RBool::IsTrue(pNode->Text());
            MO_DEBUG("Load module(%s) property. (udp_support=%d)", (TCharC*)_name, _config.udpSupport);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "udp_host")){
            // 读取广播主机名称
            pNode->GetText(_config.udpHost, sizeof(_config.udpHost));
            MO_DEBUG("Load module(%s) property. (udp_host=%s)", (TCharC*)_name, _config.udpHost);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "udp_port")){
            // 读取广播主机端口
            _config.udpPort = (TUint16)pNode->TextAsInt();
            MO_DEBUG("Load module(%s) property. (udp_port=%d)", (TCharC*)_name, _config.udpPort);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "limit_socket_count")){
            // 读取网络链接限制
            _config.limitSocketCount = pNode->TextAsInt();
            MO_DEBUG("Load module(%s) property. (limit_socket_count=%d)", (TCharC*)_name, _config.limitSocketCount);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "limit_client_count")){
            // 读取客户端数量限制
            _config.limitClientCount = pNode->TextAsInt();
            MO_DEBUG("Load module(%s) property. (limit_socket_count=%d)", (TCharC*)_name, _config.limitSocketCount);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "receive_first_timeout")){
            // 读取网络首次接收超时
            _config.receiveFirstTimeout = pNode->TextAsInt() * 1000;
            MO_DEBUG("Load module(%s) property. (receive_first_timeout=%d)", (TCharC*)_name, _config.receiveFirstTimeout);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "receive_timeout")){
            // 读取网络接收超时
            _config.receiveTimeout = pNode->TextAsInt() * 1000;
            MO_DEBUG("Load module(%s) property. (receive_timeout=%d)", (TCharC*)_name, _config.receiveTimeout);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "send_timeout")){
            // 读取网络发送超时
            _config.sendTimeout = pNode->TextAsInt() * 1000;
            MO_DEBUG("Load module(%s) property. (send_timeout=%d)", (TCharC*)_name, _config.sendTimeout);
         }else if(pNode->IsAttribute(MO_PTR_NAME, "receive_thread_count")){
            // 读取接收线程总数
            _config.receiveThreadCount = pNode->TextAsInt();
            MO_DEBUG("Load module(%s) property. (receive_thread_count=%d)", (TCharC*)_name, _config.receiveThreadCount);
         }
      }else if(pNode->IsName("Host")){
         // 读取网络连接设置
         SNetHost host;
         host.host = pNode->Get("host");
         host.port = (TUint16)pNode->GetAsInt("port");
         _config.hosts.Push(host);
         MO_DEBUG("Load module(%s) property. (host=%s, port=%d)", (TCharC*)_name, (TCharC*)host.host, host.port);
      }
   }
   // 设置关联数据
...
FNetBufferedSocket* FNetTransferService::InnerAddSocket(SNetSocketInfo& info){
   ...
   MO_DEBUG_INFO("Add net socket. (socket=0x%08X, poll=0x%04X(%d)|0x%04X(%d), host=%s:%d, handle=%d, index=%d:%d)",
         pSocket,
...
TBool FNetTransferService::CloseSocketWithNotify(FNetBufferedSocket* pSocket){
   ...
   if(connected){
      ...
      MO_DEBUG_INFO("Remove client soclet. (socket=0x%08X, poll=0x%04X(%d)|0x%04X(%d), host=%s:%d, handle=%d(%s))",
...
TBool FNetTransferService::CloseSocketWithoutNotify(FNetBufferedSocket* pSocket){
   ...
   if(connected){
      // 断开链接
      MO_DEBUG_INFO("Remove client soclet. (socket=0x%08X, poll=0x%04X|0x%04X, host=%s:%d, handle=%d(%s))",
...
TBool FNetTransferService::SendSocketRouter(FNetBufferedSocket* pSocket, TNetRouter* pRouter){
   ...
   if(!_pInputQueue->PushRouter(&transfer)){
      ...
      MO_FATAL("Send transfer socket message failure.\n%s%s",
            pRouter->Dump(dump, MO_FS_DUMP_LENGTH),
            pRouter->DumpMemory(format, MO_FS_DUMP_LENGTH));
#endif
      return EFalse;
   }
   MO_DEBUG("Send socket router. (length=%d)", transfer.NetHead().Length());
   return ETrue;
}
...
TBool FNetTransferService::SendSocketTransfer(FNetBufferedSocket* pSocket, TNetTransfer* pTransfer){
   ...
   MO_DEBUG("Send socket transfer. (length=%d)", pTransfer->NetHead().Length());
   return ETrue;
}
...
TResult FNetTransferService::StatisticsRefresh(){
   ...
   MO_INFO("Transfer service statistics.", (TCharC*)dump);
   return ESuccess;
}
