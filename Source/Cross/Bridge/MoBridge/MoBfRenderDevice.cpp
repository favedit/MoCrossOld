...
MO_CLASS_IMPLEMENT_INHERITS(FBfRenderDevice, FRenderDevice);
...
FBfRenderDevice::FBfRenderDevice(){
   ...
   MO_CLEAR(_piDevice);
   // 注册类集合
   _pClassFactory->Register(MO_RENDEROBJECT_SHADERBUFFER,    FBfRenderShaderBuffer::Class());
   _pClassFactory->Register(MO_RENDEROBJECT_SHADERATTRIBUTE, FRenderShaderAttribute::Class());
   _pClassFactory->Register(MO_RENDEROBJECT_SHADERPARAMETER, FBfRenderShaderParameter::Class());
   _pClassFactory->Register(MO_RENDEROBJECT_SHADERSAMPLER,   FRenderShaderSampler::Class());
   _pClassFactory->Register(MO_RENDEROBJECT_LAYOUT,          FBfRenderLayout::Class());
   //
...
// <T>更新环境。</T>
//============================================================
TBool FBfRenderDevice::UpdateContext(){
   TBool result = EFalse;
   // 设置激活的程序
   if(_pActiveProgram != _pProgram){
      //FBfRenderProgram* pProgrom = (FBfRenderProgram*)_pProgram;
...
TResult FBfRenderDevice::Setup(){
   ...
   //............................................................
   _defaultRenderTarget = FBfRenderTarget::InstanceCreate();
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::Suspend(){
   // 暂停处理
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::Resume(){
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::CheckError(TCharC* pCode, TCharC* pMessage, ...){
   // 获得错误原因
   TBool result = EFalse;
//   GLenum errorCode = 0;
...
// @return 顶点缓冲
//============================================================
FRenderVertexBuffer* FBfRenderDevice::CreateVertexBuffer(FClass* pClass){
   FRenderVertexBuffer* pVertexBuffer = FBfRenderVertexBuffer::InstanceCreate();
...
// @return 索引缓冲
//============================================================
FRenderIndexBuffer* FBfRenderDevice::CreateIndexBuffer(FClass* pClass){
   FRenderIndexBuffer* pIndexBuffer = FBfRenderIndexBuffer::InstanceCreate();
...
// @return 程序
//============================================================
FRenderProgram* FBfRenderDevice::CreateProgrom(FClass* pClass){
   FBfRenderProgram* pProgram = FBfRenderProgram::InstanceCreate();
...
// @return 渲染目标
//============================================================
FRenderTarget* FBfRenderDevice::CreateRenderTarget(FClass* pClass){
   FBfRenderTarget* pRenderTarget = FBfRenderTarget::InstanceCreate();
...
// @return 纹理
//============================================================
FRenderFlatTexture* FBfRenderDevice::CreateFlatTexture(FClass* pClass){
   FBfRenderFlatTexture* pTexture = FBfRenderFlatTexture::InstanceCreate();
...
// @return 纹理
//============================================================
FRenderCubeTexture* FBfRenderDevice::CreateCubeTexture(FClass* pClass){
   FBfRenderCubeTexture* pTexture = FBfRenderCubeTexture::InstanceCreate();
   pTexture->SetDevice(this);
   _storageTextures.Push(pTexture);
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::Clear(TFloat red, TFloat green, TFloat blue, TFloat alpha, TFloat depth){
   MO_CHECK(_pActiveRenderTarget, return ENull);
   FBfRenderTarget* pRenderTarget = _pActiveRenderTarget->Convert<FBfRenderTarget>();
   // 清空颜色
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetBackBuffer(TInt width, TInt height, TInt antiAlias, TBool depthed){
   return ESuccess;
}
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetFillMode(ERenderFillMode fillModeCd){
   // 检查状态
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetDepthMode(TBool depth, ERenderDepthMode depthCd){
   // 检查状态
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetCullingMode(TBool cull, ERenderCullMode cullCd){
   // 检查状态
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetBlendFactors(TBool blend, ERenderBlendMode sourceCd, ERenderBlendMode targetCd){
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetScissorRectangle(TInt left, TInt top, TInt width, TInt height){
   D3D10_RECT rect;
   rect.left = left;
   rect.top = top;
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetRenderTarget(FRenderTarget* pRenderTarget){
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetProgram(FRenderProgram* pProgram){
   ...
   if(pProgram != NULL){
      // 设置顶点脚本
      FBfRenderVertexShader* pVertexShader = pProgram->VertexShader()->Convert<FBfRenderVertexShader>();
      ...
      // 设置像素脚本
      FBfRenderFragmentShader* pFragmentShader = pProgram->FragmentShader()->Convert<FBfRenderFragmentShader>();
      ...
      // 设置输入层次
      FBfRenderProgram* pRenderProgram = pProgram->Convert<FBfRenderProgram>();
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::SetLayout(FRenderLayout* pLayout){
   MO_CHECK(pLayout, return ENull);
   // 获得顶点流
   TResult result = ESuccess;
   FBfRenderLayout* pRenderLayout = pLayout->Convert<FBfRenderLayout>();
   // 获得信息
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindConstData(ERenderShader shaderCd, TInt slot, ERenderShaderParameterFormat formatCd, TAnyC* pData, TInt length){
   // 检查变更
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindConstFloat3(ERenderShader shaderCd, TInt slot, TFloat x, TFloat y, TFloat z){
   MO_FATAL_UNSUPPORT();
   return ESuccess;
}
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindConstFloat4(ERenderShader shaderCd, TInt slot, TFloat x, TFloat y, TFloat z, TFloat w){
   MO_FATAL_UNSUPPORT();
   return ESuccess;
}
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindConstMatrix3x3(ERenderShader shaderCd, TInt slot, const SFloatMatrix3d& matrix){
   MO_FATAL_UNSUPPORT();
   return ESuccess;
}
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindConstMatrix4x4(ERenderShader shaderCd, TInt slot, const SFloatMatrix3d& matrix){
   MO_FATAL_UNSUPPORT();
   return ESuccess;
}
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindShaderBuffer(FRenderShaderBuffer* pBuffer){
   MO_CHECK(pBuffer, return ENull);
   if(!pBuffer->IsStatusUsed()){
      return EContinue;
   }
   // 更新数据
   FBfRenderShaderBuffer* pRenderBuffer = pBuffer->Convert<FBfRenderShaderBuffer>();
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindVertexBuffer(TInt slot, FRenderVertexBuffer* pVertexBuffer, TInt offset, ERenderVertexFormat formatCd){
   MO_ERROR_CHECK(slot >= 0, return EFailure, "Slot value is invalid. (slot=%d)", slot);
   // 获得顶点流
   TResult result = ESuccess;
   FBfRenderVertexBuffer* pBuffer = pVertexBuffer->Convert<FBfRenderVertexBuffer>();
   //// 获得信息
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::BindTexture(TInt slot, FRenderTexture* pTexture){
   TResult result = ESuccess;
   //............................................................
   // 绑定纹理
   ...
   switch (textureCd){
      case ERenderTexture_Flat2d:{
         FBfRenderFlatTexture* pFlatTexture = (FBfRenderFlatTexture*)pTexture;
      ...
      case ERenderTexture_Cube:{
         FBfRenderCubeTexture* pCubeTexture = (FBfRenderCubeTexture*)pTexture;
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::DrawTriangles(FRenderIndexBuffer* pIndexBuffer, TInt offset, TInt count){
   ...
   _pProgram->DrawBegin();
   // 设置索引流
   FBfRenderIndexBuffer* pBuffer = pIndexBuffer->Convert<FBfRenderIndexBuffer>();
   ID3D10Buffer* piBuffer = pBuffer->NativeBuffer();
   if(piBuffer == NULL){
...
// @return 处理结果
//============================================================
TResult FBfRenderDevice::Present(){
